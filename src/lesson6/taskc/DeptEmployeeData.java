...
public class DeptEmployeeData {
	...
	private void sort(DeptEmployee[] deptEmployees) {
		...
		Arrays.sort(deptEmployees,new SalaryComparator());
	}

	public static void main(String[] args) {
		DeptEmployee[] deptEmployees = DeptEmployeeData.getDeptData();
		DeptEmployeeData deptEmployee = new DeptEmployeeData();
		Arrays.sort(deptEmployees,(e1,e2)->Double.compare(e2.getSalary(),e1.getSalary()));
		//deptEmployee.sort(deptEmployees);
		for (DeptEmployee e: deptEmployees
			 ) {
